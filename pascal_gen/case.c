...
main(int argc, char *argv[])
{
    ...
    printf("type\n");
    printf("  PSpecial_case=^special_case;\n");
    printf("  special_case = record\n");
    printf("    cp: array of uint_least32_t;\n");
    printf("    cplen: size_t;\n");
    printf("  end;\n\n");
	...
	printf("upper_special: array[0..%u] of special_case = (\n",sclen-1);
	for (i = 0; i < sclen; i++) {
		printf("\t(\n");

		printf("\t\tcp: [");
		for (j = 0; j < sc[i].upper.cplen; j++) {
			printf(" $%06X", sc[i].upper.cp[j]);
	...
	printf("lower_special: array[0..%u] of special_case = (\n",sclen-1);
	for (i = 0; i < sclen; i++) {
		printf("\t(\n");

		printf("\t\tcp: [");
		for (j = 0; j < sc[i].lower.cplen; j++) {
			printf(" $%06X", sc[i].lower.cp[j]);
	...
	printf("title_special: array[0..%u] of special_case = (\n",sclen-1);
	for (i = 0; i < sclen; i++) {
		printf("\t(\n");

		printf("\t\tcp: [");
		for (j = 0; j < sc[i].title.cplen; j++) {
			printf(" $%06X", sc[i].title.cp[j]);
